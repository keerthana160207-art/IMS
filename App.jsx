...
export default function App() {
  ...
  if (loggedIn && role === "Admin")
    return <AdminPage onLogout={handleLogout} isDark={isDark} toggleTheme={toggleTheme} t={t} />;

  return <LoginPage onLogin={handleLogin} role={role} setRole={setRole} isDark={isDark} toggleTheme={toggleTheme} t={t} />;
}

function AdminPage({ onLogout, isDark, toggleTheme, t }) {
  return (
    <div style={{ minHeight: "100vh", background: t.bg, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16, fontFamily: "'Segoe UI', sans-serif", color: t.text, transition: "all 0.3s" }}>
      <div style={{ position: "fixed", top: 20, right: 24 }}>
        <ThemeToggle isDark={isDark} toggleTheme={toggleTheme} t={t} />
      </div>
      <div style={{ fontSize: 56 }}>⚙️</div>
      <div style={{ fontSize: 26, fontWeight: 700 }}>Admin Dashboard</div>
      <div style={{ fontSize: 14, color: t.subtext }}>Admin portal coming soon.</div>
      <button onClick={onLogout} style={{ marginTop: 12, padding: "10px 24px", background: "linear-gradient(135deg,#ef4444,#dc2626)", border: "none", borderRadius: 8, color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
        🚪 Logout
      </button>
    </div>
  );
